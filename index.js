...
const fs = require('fs');
const path = require('path'); // BU EKSİKTİ
const fetch = require('node-fetch');
const FormData = require('form-data');
const bcrypt = require('bcrypt');
const saltRounds = 10; // bcrypt karmaşıklığı
...
try {
    const rawData = fs.readFileSync(path.join(__dirname, 'settings.json'), 'utf8');
    settings = JSON.parse(rawData);
    common = settings.common || {};
    roomSettings = settings.rooms?.[0] || {}; // rooms dizisi varsa ilk elemanı al, yoksa boş obje ata

    console.log('✅ settings.json başarıyla yüklendi.');
} catch (err) {
    console.error('❌ settings.json dosyasını okurken hata:', err);
    process.exit(1); // kritik hata
}

// Harita JSON'ları settings.json'dan çekilir
const trainingMap = JSON.stringify(common.isinma);     // Isınma haritası
const classicMap = JSON.stringify(common['2v2']);      // 2v2 haritası
const bigMap = JSON.stringify(common.stadium);         // Büyük harita (örneğin 3v3)

let currentStadium = 'isinma'; // Başlangıçta ısınma haritası
let bigMapObj = JSON.parse(trainingMap); // ısınma haritası objesi
...
db.connect((err) => {
  ...
  let playerLoginTimeouts = {}; // Kullanıcıların giriş sürelerini takip etmek için
  let playerLoginAttempts = {}; // Kullanıcıların giriş süresi boyunca mesaj atma durumlarını takip etmek için
  let playerAuthConn = {}; // Kullanıcıların auth ve conn değerlerini saklamak için
  let loggedInPlayers = {}; // Giriş yapmış kullanıcıları takip etmek için


/* ======= ODA AYARLARI ======= */

// Gerekli ayarları çek
const roomName = roomSettings.roomName || 'Default Room';
const maxPlayers = common.playerCount || 12;
const roomPublic = common.public || false;
const token = roomSettings.token || ''; // Token ayarı settings.json'dan
const roomWebhook = roomSettings.roomWebhook || '';
const gameWebhook = roomSettings.gameWebhook || '';
const fetchRecordingVariable = common.fetchRecordingVariable ?? true;
const timeLimit = common.timeLimit || 3;
const scoreLimit = common.scoreLimit || 3;

// Master şifre oluştur
const masterPassword = 10000 + Math.floor(Math.random() * 90000);
...
let gameConfig = {
  ...
  public: roomPublic,
  noPlayer: common.noPlayer || true,
  token: token.length === 39 ? token : undefined,
  geo: roomSettings.geo || undefined,
  proxy: roomSettings.proxy || undefined,
};

HaxballJS.then(HBInit => {
  const room = HBInit(gameConfig);

  // Ayarları room içine yerleştir
  room.settings = {
    ...roomSettings,
    ...common
  };
  ...
  // Ek bilgiler
  room.roomPassword = common.roomPassword || '';
  room.masterPassword = masterPassword;
  room.fetchRecording = fetchRecordingVariable;
  room.webhooks = {
    room: roomWebhook,
    game: gameWebhook
  };
  ...
  currentStadium = 'isinma';
  bigMapObj = JSON.parse(trainingMap);

  // Log bilgisi: oda bağlantısı ve şifre
  room.onRoomLink = (url) => {
    console.log(`🔗 Oda Linki: ${url}`);
    console.log(`🔐 Master Şifre: ${room.masterPassword}`);
  };

  // Oda başlatıldığında bilgi ver
  room.onRoomStart = () => {
    console.log(`🚀 Oda başlatıldı: ${roomName}`);
  };
...
var debugMode = false;
var afkLimit = debugMode ? Infinity : 999;
...
function containsURL(message) {
    const urlPattern = new RegExp(`(https?://|www\\.)[\\w-]+(\\.[\\w-]+)+(/\\S*)?`, 'i');
    const defaultExtensions = ['.com', '.net', '.org', '.gg'];
    const extensionsPattern = (settings.urlExtensions || defaultExtensions).join('|').replace(/\./g, '\\.');
    const fullPattern = new RegExp(`\\b(https?://|www\\.)?[\\w-]+(${extensionsPattern})\\b`, 'i');
    return fullPattern.test(message) || urlPattern.test(message);
}
...
room.onPlayerJoin = function (player) {
    // Auth ve Conn bilgisini hem eski sistem hem yeni için sakla
    playerAuthConn[player.id] = { auth: player.auth, conn: player.conn };
    authArray[player.id] = [player.auth, player.conn];

    if (!playersAll.some(p => p.id === player.id)) {
    ...
    if (settings.webhookURL) {
        fetch(settings.webhookURL, {
            method: 'POST',
            body: JSON.stringify({
                content: `[${new Date().toLocaleString()}] ➡️ JOIN (${room.getPlayerList().length}/${settings.playerCount})\n**${player.name}** [${player.auth}] {${player.conn}}`,
                username: settings.roomName,
            }),
    ...
    room.sendAnnouncement(
        `👋 Welcome ${player.name}!\nEnter "t" for team chat, "@@name" for PM.`,
        player.id,
        welcomeColor,
        'bold',
        HaxNotification.CHAT
    ...
    // Kara liste kontrolü
    db.query('SELECT * FROM banned_member WHERE isim = ? OR auth = ? OR conn = ?', [player.name, player.auth, player.conn], (error, results) => {
        if (error) return console.error('Ban kontrol hatası:', error);

        if (results.length > 0) {
            room.kickPlayer(player.id, "Kara listede olduğunuz için atıldınız.");
            return;
        }

        // Kullanıcı kontrolü
        db.query('SELECT * FROM users WHERE username = ?', [player.name], (err, results) => {
            if (err) throw err;

            if (results.length > 0) {
                const user = results[0];
                if (user.auth === player.auth && user.conn === player.conn) {
                    const adminLevel = user.AdminLevel;
                    loggedInPlayers[player.id] = adminLevel;

                    room.sendAnnouncement('✅ Otomatik giriş başarılı.', player.id, 0x00FF00, "bold", 2);

                    if (adminLevel >= 1) {
                        room.setPlayerAdmin(player.id, true);
                        room.sendAnnouncement(`🔑 Admin ${player.name} bağlandı.`, null, announcementColor, "bold", 2);
                    }
                } else {
                    room.sendAnnouncement('⚠️ Bu isimle kayıt var. 20 saniye içinde !giriş ŞİFRE yazmalısınız.', player.id, 0xFF0000, "bold", 2);

                    playerLoginTimeouts[player.id] = setTimeout(() => {
                        room.kickPlayer(player.id, 'Giriş süresi doldu.', false);
                        delete playerLoginTimeouts[player.id];
                        delete playerLoginAttempts[player.id];
                        delete playerAuthConn[player.id];
                    }, 20000);

                    playerLoginAttempts[player.id] = true;
                }
            } else {
                room.sendAnnouncement('📋 Kayıt bulunamadı. !kayıt ŞİFRE ŞİFRE ile kayıt olabilirsiniz.', player.id, 0xFF0000, "bold", 2);
            }
        });

        // Oda doluluk kontrolü
        const currentPlayerCount = room.getPlayerList().length;
        if (currentPlayerCount >= settings.playerCount) {
            db.query('SELECT AdminLevel FROM users WHERE username = ?', [player.name], (err, results) => {
                if (err) throw err;
                if (!results[0] || results[0].AdminLevel < 1) {
                    room.kickPlayer(player.id, "⛔ Oda dolu, giriş izniniz yok.", false);
                }
            });
        }
    });
...
room.onPlayerTeamChange = function (changedPlayer, byPlayer) {
    ...
    if (AFKSet?.has(changedPlayer.id) && changedPlayer.team !== Team.SPECTATORS) {
        ...
        room.sendAnnouncement(`${changedPlayer.name} is AFK!`, null, errorColor, 'bold', HaxNotification.CHAT);
        return;
    }
    ...
    if (gameState !== State.STOP) {
        if (changedPlayer.team !== Team.SPECTATORS &&
            ...
            if (changedPlayer.team === Team.RED) teamRedStats.push(changedPlayer);
            else teamBlueStats.push(changedPlayer);
        }
    }
...
room.onPlayerLeave = function (player) {
    ...
    if (!kickFetchVariable && settings.webhookURL) {
        fetch(settings.webhookURL, {
            method: 'POST',
            body: JSON.stringify({
                content: `[${new Date().toLocaleString()}] ⬅️ LEAVE (${room.getPlayerList().length - 1}/${settings.playerCount})\n**${player.name}** [${player.auth}] {${player.conn}}`,
                username: settings.roomName,
            }),
            headers: { 'Content-Type': 'application/json' },
        }).catch(err => console.error("Webhook hatası:", err));
...
room.onPlayerKicked = function (kickedPlayer, reason, ban, byPlayer) {
    ...
    if (settings.webhookURL) {
        const kickedInfo = `**${kickedPlayer.name}** [${kickedPlayer.auth}] {${kickedPlayer.conn}}`;
        const byInfo = byPlayer ? ` by **${byPlayer.name}** [${byPlayer.auth}] {${byPlayer.conn}}` : '';
        const action = ban ? 'BAN' : 'KICK';

        fetch(settings.webhookURL, {
            method: 'POST',
            body: JSON.stringify({
                content: `[${new Date().toLocaleString()}] ⛔ ${action} (${room.getPlayerList().length}/${settings.playerCount})\n${kickedInfo} was ${action.toLowerCase()}${byInfo}`,
                username: settings.roomName,
            }),
            headers: { 'Content-Type': 'application/json' },
        }).catch(err => console.error("Webhook hatası:", err));
    ...
    // Yetki kontrolü
    const kickerLevel = loggedInPlayers[byPlayer?.id] || 0;
    const kickedIsMaster = loggedInPlayers[kickedPlayer.id] >= 10;

    if (ban && (byPlayer?.id === kickedPlayer.id || kickerLevel < 10 || kickedIsMaster || disableBans)) {
        room.clearBan(kickedPlayer.id);
        return;
    }

    if (byPlayer && kickerLevel < 5) {
        room.sendAnnouncement('⛔ Bu işlemi yapmaya yetkiniz yok.', byPlayer.id, errorColor, 'bold', HaxNotification.CHAT);
        room.setPlayerAdmin(byPlayer.id, false);
        return;
    }

    if (ban) {
        db.query('INSERT INTO banned_member (isim, auth, conn) VALUES (?, ?, ?)', [kickedPlayer.name, kickedPlayer.auth, kickedPlayer.conn], (err) => {
            if (err) console.error('Ban veritabanı hatası:', err);
            else console.log(`${kickedPlayer.name} kara listeye eklendi.`);
        });
    }
};
...
room.onPlayerChat = function (player, message) {
    ...
    // Webhook
    if (!hideClaimMessage || args[0] !== '!claim') {
        if (roomWebhook != '') {
            fetch(roomWebhook, {
                method: 'POST',
                body: JSON.stringify({
                    content: `[${getDate()}] 💬 CHAT\n**${playerName}** : ${message.replace('@', '@ ')}`,
                    username: roomName,
                }),
                headers: { 'Content-Type': 'application/json' },
            }).catch(() => {});
    ...
    // Giriş gerekliliği uyarısı
    if (playerLoginAttempts[playerId] && args[0] !== '!giriş') {
        room.sendAnnouncement('Önce giriş yapmalısın. !giriş ŞİFRE şeklinde giriş yapabilirsin.', playerId, 0xFF0000, "bold", 2);
        return false;
    }

    // Mesaj kontrolü (spam/küfür/url)
    if (!player.admin) {
        if (message.length > 100) {
            room.sendAnnouncement('Mesajınız 100 karakterden fazla olamaz.', playerId, 0xFF0000, "bold", 2);
            return false;
        }
        ...
        for (const word of badWords) {
            if (lowerMessage.includes(word)) {
                room.sendAnnouncement(`${player.name}, mesajında uygunsuz ifade tespit edildi.`, null, 0xFF0000, "bold", 2);
                return false;
            }
        }

        if (containsURL(message)) {
            room.sendAnnouncement(`${player.name}, URL paylaşımı yasaktır.`, null, 0xFF0000, "bold", 2);
            return false;
        }
    }
    ...
    if (args[0] === '!kayıt') {
        if (args.length !== 3) {
            room.sendAnnouncement('Kullanım: !kayıt ŞİFRE ŞİFRE', playerId, 0xFF0000, "bold", 2);
            return false;
        }
        const [pass1, pass2] = [args[1], args[2]];
        if (pass1 !== pass2 || pass1.length < 5) {
            room.sendAnnouncement('Şifreler aynı olmalı ve en az 5 karakter olmalı.', playerId, 0xFF0000, "bold", 2);
            return false;
        }
        db.query('SELECT * FROM users WHERE username = ?', [playerName], (err, results) => {
            if (results.length > 0) {
                room.sendAnnouncement('Bu isimle kayıt zaten var. !giriş ŞİFRE', playerId, 0xFF0000, "bold", 2);
            } else {
                const auth = playerAuthConn[playerId]?.auth;
                const conn = playerAuthConn[playerId]?.conn;
                bcrypt.hash(pass1, saltRounds, (err, hash) => {
                    db.query('INSERT INTO users (username, password, auth, conn) VALUES (?, ?, ?, ?)', [playerName, hash, auth, conn], () => {
                        room.sendAnnouncement('✅ Kayıt başarılı!', playerId, 0x00FF00, "bold", 2);
                    });
                });
            }
        });
    ...
    if (args[0] === '!giriş') {
        if (args.length !== 2) {
            room.sendAnnouncement('Kullanım: !giriş ŞİFRE', playerId, 0xFF0000, "bold", 2);
            return false;
        }
        const password = args[1];
        db.query('SELECT * FROM users WHERE username = ?', [playerName], (err, results) => {
            if (results.length === 0) {
                room.sendAnnouncement('Kayıt bulunamadı.', playerId, 0xFF0000, "bold", 2);
            } else {
                const user = results[0];
                bcrypt.compare(password, user.password, (err, match) => {
                    if (match) {
                        loggedInPlayers[playerId] = user.AdminLevel;
                        room.sendAnnouncement('✅ Giriş başarılı!', playerId, 0x00FF00, "bold", 2);
                        ...
                        const conn = playerAuthConn[playerId]?.conn;
                        if (auth && conn) {
                            db.query('UPDATE users SET auth = ?, conn = ? WHERE username = ?', [auth, conn, playerName]);
                        }
                    } else {
                        room.sendAnnouncement('❌ Hatalı şifre.', playerId, 0xFF0000, "bold", 2);
                    }
                });
            }
        });
    ...
    // Komut: !şifredeğiştir
    if (args[0] === '!şifredeğiştir') {
        const adminLevel = loggedInPlayers[playerId] || 0;
        if (adminLevel < 10) {
            room.sendAnnouncement('❌ Yetkiniz yok.', playerId, 0xFF0000, "bold", 2);
            return false;
        }
        if (args.length !== 4 || args[2] !== args[3] || args[2].length < 5) {
            room.sendAnnouncement('Kullanım: !şifredeğiştir İSİM YENİŞİFRE YENİŞİFRE', playerId, 0xFF0000, "bold", 2);
            return false;
        }
        const [targetName, newPass] = [args[1], args[2]];
        db.query('SELECT * FROM users WHERE username = ?', [targetName], (err, res) => {
            if (res.length === 0) {
                room.sendAnnouncement('❌ Kullanıcı bulunamadı.', playerId, 0xFF0000, "bold", 2);
            } else {
                bcrypt.hash(newPass, saltRounds, (err, hash) => {
                    db.query('UPDATE users SET password = ? WHERE username = ?', [hash, targetName]);
                    room.sendAnnouncement('🔐 Şifre güncellendi.', playerId, 0x00FF00, "bold", 2);
                });
            }
        });
        return false;
    ...
    // Komut: !rank
    if (message === '!rank') {
        db.query('SELECT win, lose, goal, kk, cs FROM users WHERE username = ?', [playerName], (err, res) => {
            if (res.length === 0) {
                room.sendAnnouncement('📋 Kayıt bulunamadı.', playerId, 0xFF0000, "bold");
            } else {
                const { win, lose, goal, kk, cs } = res[0];
                const msg = `🏆 Kazanma: ${win}, ❌ Kaybetme: ${lose}, ⚽ Gol: ${goal}, 🥅 KK: ${kk}, 🔒 CS: ${cs}`;
                room.sendAnnouncement(`${playerName}: ${msg}`, null, 0x00FF00, "bold");
            }
        });
        return false;
    }
    ...
    if (args[0].startsWith('!')) {
        let command = getCommand(args[0].slice(1).toLowerCase());
        if (command && commands[command].roles <= getRole(player)) {
            commands[command].function(player, message);
        } else {
            room.sendAnnouncement('⛔ Bu komutu kullanamazsınız. !help yazın.', player.id, errorColor, 'bold', 2);
        }
        return false;
    }
    ...
    // Yavaş mod
    if (slowMode > 0) {
        if (slowModeFunction(player, message)) return false;
    }

    // Mute kontrolü
    if (!player.admin && muteArray.getByAuth(authArray[player.id][0])) {
        room.sendAnnouncement('🔇 Susturuldunuz.', player.id, errorColor, 'bold');
        return false;
    }
    ...
    db.query('SELECT AdminLevel FROM users WHERE username = ?', [player.name], (err, results) => {
        ...
        let prefix = '';
        if (results.length > 0) {
            const level = results[0].AdminLevel;
            if (level === 1) color = 0xFFD700;
            if (level === 2) {
                color = 0xFF4500;
                prefix = 'Admin* ';
            }
        }
        room.sendAnnouncement(`${prefix}${player.name}: ${message}`, null, color, "bold");
    });
...
room.onTeamGoal = function (team) {
    ...
    // Duyuru gönder
    room.sendAnnouncement(goalString, null, team == Team.RED ? redColor : blueColor, null, HaxNotification.CHAT);
    ...
    // Son dokunan oyuncu kontrolü
    let scorer = lastPlayersTouched?.[0];
    if (scorer) {
        const isOwnGoal = (team === Team.RED && scorer.team === Team.BLUE) || (team === Team.BLUE && scorer.team === Team.RED);

        if (!isOwnGoal) {
            room.sendAnnouncement(`${scorer.name} harika bir gol attı! ⚽️`, null, 0x00FF00, "bold", 2);
            if (loggedInPlayers[scorer.id]) {
                db.query('UPDATE users SET goal = goal + 1 WHERE username = ?', [scorer.name], (err) => {
                    if (err) throw err;
                    console.log(`${scorer.name} gol attı, goal sayısı güncellendi.`);
                });
            }
        } else {
            room.sendAnnouncement(`${scorer.name} kendi kalesine gol attı! 🤦‍♂️`, null, 0xFF0000, "bold", 2);
            if (loggedInPlayers[scorer.id]) {
                db.query('UPDATE users SET kk = kk + 1 WHERE username = ?', [scorer.name], (err) => {
                    if (err) throw err;
                    console.log(`${scorer.name} kendi kalesine gol attı, kk sayısı güncellendi.`);
                });
            }
        }
    }

    // Skor limitine ulaşıldıysa oyunu bitir
